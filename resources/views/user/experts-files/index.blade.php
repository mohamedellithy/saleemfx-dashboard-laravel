    ...
    @if($service->not_allowed_to_user)
        <div class="row">
            <div class="container-image-chashback col-md-6 col-xs-12">
                ...
                <h2 class="title-image-chashback"> {{ __('master.expert-files') }} </h2>

                @if($service->user_not_have_order)

                    <button type="button" type-form="create" class="btn btn-warning activateServices" order-id="" SERVICES-ID="{{ $service->ID }}"> {{ __('maaster.subscrib_services') }}</button>
