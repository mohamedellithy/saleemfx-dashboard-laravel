<?php

return [
    'home'                       => 'الرئيسية',
    'services'                   => 'الخدمات',
    'about-us'                   => 'من نحن',
    'videos'                     => 'الفيديو',
    'analysis'                   => 'التحليل الفنى',
    'blog'                       => 'المدونة',
    'academy'                    => 'الاكاديمية',
    'economic-news'              => 'الاخبار الاقتصادية',
    'contact-us'                 => 'تواصل معنا',
    'be-partner'                 => 'كن شريكا',
    'login'                      => 'تسجيل الدخول',
    'register'                   => 'انساء حساب',
    'my-account'                 => 'حسباتى',
    'order-charge-my-wallet'     => 'طلبات شحن محفظتى',
    'my-orders'                  => 'طلباتى',
    'order-withdraw-my-balnc'    => 'طلبات سحب رصيدى',
    'cashback-my-accounts'       => 'كاش باك حسباتى',
    'cashback-expire'            => 'كاش باك منتهى',
    'my-services'                => 'خدماتى',
    'show-my-services'           => 'عرض خدماتى',
    'expert-files'               => 'ملفات الاكسبرتات',
    'recommended-services'       => 'خدمات التوصيات',
    'directrix-services'         => 'خدمات مؤشر سليم',
    'affiliate-marketing'        => 'التسويق بالعمولة',
    'invited-clients'            => 'دعوة العملاء',
    'affiliate-marketing-profit' => 'أرباح التسويق بالعمولة',
    'withdrawal-requests-marketing-profits' => 'طلبات سحب الارباح',
    'affiliater-marketing'       => 'مسوق بالعمولة',
    'show_my_account'            => 'عرض حسباتى',
    'create_new_account'         => 'انشاء حساب جديد',
    'edit_forex_account'         => 'تعديل حساب التداول',
    'forex_arabic_name'          => 'اسم شركة التداول',
    'forex_english_name'         => 'اسم شركة التداول بالانجليزي',
    'forex_number_account'       => 'رقم حساب شركة التداول',
    'forex_company_cost'         => 'مبلغ التداول فى شركة الفوركس',
    'company_name'               => 'اسم الشركة',
    'company_account_number'     => 'رقم حساب الشركة',
    'forex_company_cost_d'       => 'قيمة التداول فى الشركة',
    'order_status'               => 'حالة الطلب',
    'created_at'                 => 'تاريخ الانشاء',
    'edit_account'               => 'تعديل الحساب',
    'delete'                     => 'حذف',
    'pending'                    => 'قيد التنفيذ',
    'accepted'                   => 'تم الموافقة',
    'refused'                    => 'تم الرفض',
    'ended_order'                => 'انهاء الطلب',
    'show_my_wallet'             => 'عرض محفظتى',
    'charge_my_wallet'           => 'شحن محفظتى',
    'money_value'                => 'قيمة المبلغ',
    'payment_method'             => 'وسيلة الدفع',
    'my_cashback_withdraw_orders'=> 'طلبات سحب الرصيد',
    'totale_balance'             => 'اجمالى الرصيد',
    'totale_cashback'            => 'اجمالى الكاش باك',
    'total_withdraw'             => 'اجمالى المسحوب',
    'totale_withdraw_orders_pending' => 'طلبات السحب المعلقة',
    'totale_expire_cashback'     => 'الكاش باك المنتهى',
    'totale_available_cashback'  => 'المتبقي من الكاش باك',
    'money_needed'               => 'المبلغ المطلوب سحبه',
    'wallet'                     => 'المحفظة',
    'wallet_account_number'      => 'رقم حساب المحفظة',
    'cancel_order'               => 'الغاء الطلب',
    'not_found_cashback'         => 'لايوجد كاش باك',
    'subscrib_services'          => 'الاشتراك فى الخدمة',
    'cashbacks'                  => 'الاسترداد النقدي',
    'active-account'             => 'تفعيل الحساب',
    'accept_request_form_admin'  => 'بانتظار قبول الطلب من المسؤل فى الموقع',
    'alert-cashback'             => 'تنويه : جميع ارصدة الكاش باك التي لم يتم سحبها خلال 3 شهور من تاريخ ترصيدها، يتم حذفها نهائيا حسب القوانين المتبعة في مجموعة سليم',
    'cashback_value'             => 'قيمة الكاش باك',
    'cash-back-account-month'    => 'شهر حساب الكاش باك ',
    'cashback_available'         => 'كاش باك مسموح',
    'cashback_expire_not_found'  => 'لايوجد كاش باك منتهي فى حسابك ',
    'my-services-subscribe'      => 'الخدمات المشترك بها',
    'other-services'             => 'الخدمات اخري',
    'no-services-available' => 'لا يوجد اى خدمات متاحة ',
    'activate' => 'تفعيل',
    'waiting-for-request-review' => 'بانتظار مراجعة الطلب ',
    'subscription-expiration-date' => 'تاريخ انتهاء الاشتراك',
    'renew-your-subscription-to-the-service' => 'تجديد الاشتراك فى الخدمة ',
    'subscription-to-the-service' => ' الاشتراك فى الخدمة',
    'forex_account' => 'حساب التداول ',
    'service-subscription-price' => 'سعر الاشتراك فى الخدمة ',
    'months_account' => 'عدد الاشهر',
    'month' => 'شهر',
    'totale_cost' => 'اجمالى التكلفة',
    'minimum-subscription-for-the-service' => 'الحد الادنى للاشتراك فى الخدمة',
    'descrip_forex_value' => 'قيمة مبالغ التداول أقل من الحد الادنى للاشتراك في الخدمة ',
    'expert-file-name' => 'اسم ملف الاكسبرت',
    'description-of-the-expert-file' => 'وصف ملف الاكسبرت',
    'download' => 'تحميل',
    'join-recommendation-services' => 'الانضمام لخدمة التوصيات',
    'request' => 'طلب',
    'directrix_saleem_services' => 'خدمات مؤشر سليم',
    'file_name_of_saleem_index' => 'اسم ملف مؤشر سليم',
    'create_new_account_affiliate' => 'انشاء حساب مسوق الكترونى',
    'Link-to-your-affiliate-account' => 'رابط حسابك على الافيليت',
    'invite_your_friends' => 'قم بدعوة أصدقائك و أقاربك عن طريق البريد الالكترونى ',
    'Write-your-email-here' => 'قم بكتابة بريد الالكتروني هنا ',
    'send-invitation' => 'ارسال الدعوة',
    'user-name' => 'اسم المستخدم',
    'email' => 'البريد الالكترونى',
    'total-marketing-profit' => 'اجمالى أرباح التسويق',
    'affiliate-marketing-withdrawal-request' => 'طلب سحب أرباح التسويق بالعمولة',
    'affiliate-marketing-earnings' => 'أرباح التسويق بالعمولة',
    'amount-required-for-withdrawal' => 'قيمة المبلغ المطلوب للسحب ',
    'send-request' => 'ارسال الطلب',
    'alert_withdraw' => 'لا يمكنك اجراء عمليات سحب من الرصيد رصيدك الحالى غير كافى لاتمام عملية السحب',
    'invitee' => 'مدعو',
    'profit_value' => 'قيمة الربح',
     'the-date-of-adding-the-profit' => 'تاريخ اضافة الربح',
     'with-Seleem-you-are-in-the-right' => 'مع سليم انت فى السليم',
     'email_or_username' => 'البريد الالكترونى أو اسم المستخدم ',
     'Don-t-have-an-account_Sign_up' => 'لا تمتلك حساب ؟ قم بالتسجيل',
     'did-you-forget-your-password' => 'هل نسيت كلمة المرور ؟',
     'In-the-affiliate-marketing-program' => 'فى برنامج التسويق بالعمولة',
     'first-name' => 'الاسم الاول',
     'last-name' => 'الاسم الاخير',
     'phone-number' => 'رقم الجوال',
     'email' => 'البريد الالكترونى',
     'password' => 'كلمة المرور',
     'confirm_password' => 'تأكيد كلمة المرور',
     'do-you-have-an-account-sign-in' => 'هل تمتلك حساب ؟ سجل الدخول',
     'confirm-your-email' => 'قم بتأكيد بريدك الاكترونى',
     'new-verification-link-has-been-sent-to-your-email-address' => 'تم إرسال رابط تحقق جديد إلى عنوان بريدك الإلكتروني',
     'before-proceeding-please-check-your-email-for-verification-link' => 'قبل المتابعة ، يرجى التحقق من بريدك الإلكتروني للحصول على رابط التحقق',
     'If-you-did-not-receive-the-email' => 'إذا لم تستلم البريد الإلكتروني',
     'click-here-to-order-another' => 'انقر هنا لطلب آخر',
     'send' => 'ارسال',
     'Forgot-your-password' => 'نسيت كلمة المرور',
     'set-password' => 'ضبط كلمة المرور',
     'please-confirm-the-password' => 'من فضلك قم بتأكيد كلمة المرور',
     '' => '',
     '' => '',
     '' => ''


];
